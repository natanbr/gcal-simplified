...
// ⚡ Bolt Performance: Extract item rendering into a React.memo component to prevent
// full 35-day grid re-renders when parent state (like currentDate) changes.
const MonthDayCell: React.FC<MonthDayCellProps> = React.memo(({ day, dayEvents, isToday, isCurrentMonth, onEventClick }) => {
    // ⚡ Bolt Performance: Cache expensive getEventColorStyles mapping.
    // This prevents re-calculating styles for all events if the cell re-renders.
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto space-y-1.5 no-scrollbar">
                {eventStyles.map(({ event, style, className }) => (
                    <button
                        key={event.id}
