...
import { BOARD_CELL_PITCH, BOARD_PADDING } from './types';
...
function setup(gameState: BlocksGameState) {
    const view = renderCanvas(gameState);
    return { ...view, item: draggableItem(view.container) };
}
...
describe('the projection is recomputed on every move, not just the first', () => {
    ...
    it('lets the child correct a forgiveness snap by moving on', () => {
        const { item, placeShape, ghost } = setup(stateWith(DOT, { grid: blockedGrid([4, 4]) }));
        ...
        expect(ghost()!.style.gridRowStart).toBe('6'); // 1-indexed: row 5
        expect(ghost()!.style.gridColumnStart).toBe('5');
        ...
        expect(ghost()!.style.gridRowStart).toBe('2');
        expect(ghost()!.style.gridColumnStart).toBe('2');
    ...
    it('turns a stale green ghost red when the shape moves onto a blocked cell', () => {
        ...
        fireEvent.pointerMove(window, { ...cellCentre(1, 1), pointerId: 1 });
        expect(ghost()!.style.background).toContain('74, 222, 128'); // green
        ...
        expect(ghost()!.style.background).toContain('239, 68, 68'); // red
        expect(ghost()!.style.gridRowStart).toBe('5');
...
describe('the projection is recomputed when the board changes under a still finger', () => {
    ...
    it('turns green red when a meteor lands under the ghost during a line clear', () => {
        ...
        const { item, placeShape, ghost, rerenderWith } = setup(stateWith(DOT));
        fireEvent.pointerDown(item, grabCorner(1));
        ...
        expect(ghost()!.style.background).toContain('239, 68, 68'); // red
        expect(ghost()!.style.gridRowStart).toBe('4');
...
describe('the board origin is measured from the board box model, not assumed', () => {
    ...
    const SNAPPED_FINGER = {
        clientX: BOARD_LEFT + 2 + BOARD_PADDING + 1.5 * BOARD_CELL_PITCH + HALF_CELL,
        clientY: BOARD_TOP + 2 + BOARD_PADDING + 1.5 * BOARD_CELL_PITCH + HALF_CELL,
        pointerId: 1,
    };
     ...
     *
     *   proxyOrigin.y = clientY − grabRow·BOARD_CELL_PITCH − CELL_DISPLAY_SIZE/2 − lift
...
describe('closing the game mid-drag', () => {
    ...
    it('releases the window listeners, so a later pointerup cannot place into a dead game', () => {
        const { item, placeShape, unmount, ghost } = setup(stateWith(DOT));
        fireEvent.pointerDown(item, grabCorner(1));
        fireEvent.pointerMove(window, { ...cellCentre(3, 3), pointerId: 1 });
        // Without a live drag over a placeable cell the listeners are never
        // attached, and the not-called assertion below would pass on its own.
        expect(ghost()!.style.background, 'precondition: a green ghost is live').toContain('74, 222, 128');
