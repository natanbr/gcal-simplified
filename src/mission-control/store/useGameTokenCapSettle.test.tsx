...
const SETTLE = /removed at update/;
...
describe('loading over the cap — happy path', () => {
    it('5 game tokens plus a Quick-Game goal load as 4, with one log line from the system', async () => {
        ...
        expect(lines[0]).toMatchObject({ source: 'system', delta: -1, gameTokens: MAX_GAME_TOKENS - 1 });
        expect(lines[0].message).toBe(
            '1 game token removed at update: 5 game tokens plus 1 Quick-Game goal is over the 5-token cap',
        );
    });
    ...
    it('the line reaches the audit trail, like every other log entry', async () => {
        ...
        expect(audited[0]).toMatchObject({ src: 'system', d: -1, game: MAX_GAME_TOKENS - 1 });
    });
    ...
    it('counts every goal: 5 tokens plus 2 Quick-Game goals load as 3, one line for both', async () => {
        ...
        expect(settleLines()).toHaveLength(1);
        expect(settleLines()[0]).toMatchObject({ delta: -2 });
        expect(settleLines()[0].message).toMatch(/^2 game tokens removed at update: 5 game tokens plus 2 Quick-Game goals/);
    });
});
