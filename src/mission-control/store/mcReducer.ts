...
function _mcReducer(state: MCState, action: MCAction): MCState {
    const bankResult = bankReducer(state, action);
    if (bankResult !== undefined) return bankResult;

    const missionResult = missionReducer(state, action);
    if (missionResult !== undefined) return missionResult;

    const responsibilityResult = responsibilityReducer(state, action);
    if (responsibilityResult !== undefined) return responsibilityResult;

    const settingsResult = settingsReducer(state, action);
    if (settingsResult !== undefined) return settingsResult;

    const gameResult = gameReducer(state, action);
    if (gameResult !== undefined) return gameResult;

    const systemResult = systemReducer(state, action);
