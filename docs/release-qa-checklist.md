...
- [ ] **3.5.12** `[claude]` A scheduler-started mission inside its window (evening "Auto-trigger at" a few minutes ago, today's evening not yet recorded): the phone's Stop → "⏹️ Mission stopped" and it stays stopped (the desktop has no stop gesture) (currently fails: bug 1, fix in progress: it restarts at once with a fresh timer, Who "⏰ CLOCK").
