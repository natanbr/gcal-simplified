...
npm run tsc              # Type-check: app config, tsconfig.test.json (unit tests), root *.config.ts
...
4. `npm run tsc` exits with 0 errors — it checks every file under src/, electron/ and e2e/, unit tests
   included, plus the root `*.config.ts` files (guarded by `src/__tests__/typecheck-coverage.test.ts`).
   Those configs live in `tsconfig.node.json`, which plain `tsc` never builds because it is a referenced
   project, so the script runs it by name; its `--noEmit` stops an emitted `vite.config.js` shadowing
   `vite.config.ts`, and `--composite false` stops a `tsconfig.node.tsbuildinfo` landing in the repo root
   (both flags guarded by `src/__tests__/typescript-strict-config.test.ts`). `npm run build` type-checks
   the app config only
