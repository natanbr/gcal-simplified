...
## 2025-05-24 - [Weak Randomness in Security Contexts]
**Vulnerability:** `Math.random()` was used to generate security-sensitive values like the `remoteKey` for the Supabase Realtime channel in `electron/remote-bridge.ts`. `Math.random()` is a pseudo-random number generator and is not cryptographically secure, making the keys predictable and potentially allowing unauthorized remote access. It was also used to generate IDs in `electron/api.ts` and `src/mission-control/store/useMCStore.tsx`.
**Learning:** Functions like `Math.random()` are not designed for security and their output can be predicted by an attacker. They should never be used for generating keys, tokens, or unique identifiers where unpredictability is a security requirement.
**Prevention:** Always use cryptographically secure random number generators (CSPRNG). In Node.js environments, use `crypto.randomBytes()` or `crypto.randomUUID()` from the `node:crypto` module. In browser environments, use `self.crypto.randomUUID()` or `self.crypto.getRandomValues()`.
