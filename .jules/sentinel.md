...
## 2025-02-28 - Replace Predictable PRNG for Security Identifiers
**Vulnerability:** Predictable PRNG (`Math.random()`) was being used to generate fallback identifiers in API events, activity log records, and critically, the remote control bridge token (`remoteKey`). `Math.random()` provides no cryptographic strength, leading to potential token predictability and unauthorized access to remote control features.
**Learning:** Legacy ID generation (`Math.random().toString(36)...`) was incorrectly applied in a security-sensitive context. Because `Math.random()` values are deterministic based on engine state, attackers could theoretically deduce the internal state or guess upcoming session tokens if they observed previous values.
**Prevention:** Always use cryptographically secure random number generators (CSPRNG) like `crypto.randomBytes().toString('hex')` or `crypto.randomUUID()` (`self.crypto.randomUUID()` in the browser/renderer) for any token, key, or sensitive ID generation across both the renderer and main processes.
