...
## 2026-07-13 - [Framer Motion CPU Optimization]\n**Learning:** In Framer Motion, placing an infinite loop (e.g., `transition: { repeat: Infinity }`) strictly at the root level of a motion component forces the browser to run a continuous 60fps loop via JS, even when the animation condition is false or visually static, causing high background CPU usage.\n**Action:** When conditionally animating components using Framer Motion, move the infinite transition property inside the conditional `animate` object rather than setting it as a top-level prop. This ensures the JS thread is only consumed when the animation is actively needed.
