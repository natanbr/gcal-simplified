...
**…and roll nothing in it either: seed outside, generator inside.** The same replay makes `Math.random()` inside an updater re-roll, so the two runs commit different state and the second replaces the first. The line clear's meteors landed in different cells (2026-09-18, a real bug: StrictMode's discarded run still *scheduled* a timer, so dev saw it). Every deal `useBlocksGame` made inside an updater could likewise swap the shapes in the tray after one drop (2026-09-23) — that one was reasoned out from the meteor bug, never observed, and is not reachable today — but mind *why*, because the tempting reason is wrong: it is not that the app has no lower-priority writers (in React 18 with `createRoot`, ANY `setState` outside a discrete event is default-lane — a `setTimeout`, a `.then`, a passive effect; `startTransition` is only the loudest source). `useBlocksGame` has two. Neither is pending *at the moment a shape drops*, which is a claim about timing that one new deferred write invalidates. StrictMode double-runs the updater but commits once, so dev shows nothing. Fixed anyway, because it costs four lines and the next deferred update makes it live. Roll a `seed: number` outside and build `seededRandom(seed)` *inside*. Passing the generator is the trap: it is stateful, so the replay continues the sequence rather than repeating it and the symptom survives the "fix" — which is why `refillBank`, `refreshRescue` and `resolvePendingClear` take the number, not an `Rng` (`rng.ts` is its home; neither the clear nor the dealer owns it). Drop the `= Math.random` defaults on the functions an updater calls, so a forgotten argument is a type error — done for the dealer, still outstanding for `applyClearEffects`/`spawnObstacles` in `types.ts`, whose ~16 test call sites rely on the default. **Testing it:** record every committed frame in a layout effect and assert the frames are identical — but that alone is not a test. Two commits prove two lanes rendered, not two updater runs, so a harness that stops forcing the replay keeps passing with the bug restored (proven by mutation against `refill.test.tsx` and `line-clear.test.tsx`, which both still pass that way). Add a *sensitivity witness*: partial-mock the dealer and assert it was called twice. Safe only in a file that never uses StrictMode, where the discarded run would be counted too. See `useBlocksGame.deal-replay.test.tsx`.
