...
- **Global setup is minimal**: `src/test/setup.ts` only imports `@testing-library/jest-dom` and stubs `IntersectionObserver`. There is **no `src/__mocks__/` and no `e2e/fixtures/`.** The only shared fixture is `src/mock/events.ts`. If you need a shared mock, you are creating that location — say so, don't silently assume it exists.
