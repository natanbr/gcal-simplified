...
- **Global setup is minimal**: `src/test/setup.ts` only imports `@testing-library/jest-dom` and stubs `IntersectionObserver`. There is **no `src/__mocks__/` and no `e2e/fixtures/`.** Shared unit fixtures are `src/mock/events.ts` and per-module test kits beside the API they fake — `src/mission-control/games/quiz/quizTestKit.ts` exports `stubEngine()` for any game that takes a `QuizEngineApi`, and `src/mission-control/games/blocks/dragTestKit.ts` holds the Space Rescue drag setup (board geometry, shapes, `renderCanvas`; its DOM-free half is `dragFixtures.ts`); import them instead of hand-rolling a copy. If you need a shared mock that does not exist yet, you are creating that location — say so, don't silently assume it exists.
